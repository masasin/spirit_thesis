...
\chapter{General components}
  \section{Environment}
    ...
    Communication with the drone is achieved using the \texttt{\detokenize{ardrone_autonomy}} package from Simon Fraser University's Autonomy Laboratory.
...
\label{sec:poses}
  ...
  \section{Motion capture}
    ...
    The data is received by the operating station, interpreted by \gls{ros}'s \texttt{\detokenize{mocap_optitrack}} package, and republished to \texttt{ardrone}'s the \texttt{\detokenize{pose}} and \texttt{\detokenize{ground_pose}} nodes.
